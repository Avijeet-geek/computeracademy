...
class Plans extends MX_Controller
{
  ...
  PRIMARY KEY  (`menu_id`)
    ...
    function save_data()
    {
        ...
        //else
        $this->form_validation->set_rules("c_id","Child Services Required","required|trim");
        $this->form_validation->set_rules("name","Name","required|trim");
    ...
    function view()
    {
        ...
        if (isset($_GET['data']))
	        $select=$_GET['data'];
	    else $select="services.title,services.id,plans.p_id,plans.plan,plans.price,plans.description,plans.status";    
