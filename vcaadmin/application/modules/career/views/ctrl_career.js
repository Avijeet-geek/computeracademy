...
app.controller('ctrl_career',function($scope,$http){
	$http.get('login/check_valid_session').success (function(data) {if(data!=1){window.location.assign('<?=site_url("login")?>');}});
	
	$scope.loader=function(){
		$http.get("career/view_data").success(function(data){
			console.log(data);
			$scope.datadb=data;
		})
	}
	...
	$scope.delete_data=function(id)
	{
		if(confirm("Deleting Contact may hamper your data associated with it."))
		{
			...
			{
				$http.get("career/delete_data?id="+id).success(function(data){
					console.log(data);
					if(data=="1")
					{
