...
class Socialmedia extends MX_Controller
{
    ...
    function save_data()
    {
        ...
        $this->form_validation->set_rules("urlpage","Url","required");        
        if ($this->form_validation->run()==TRUE)
        {
            $data['title']=$_POST['title'];
            $data['urlpage']=$_POST['urlpage'];
            ...
            if (isset($_POST['id']) && $_POST['id'])//update
            {
                ...
                $where['socialmedia_id']=$_POST['id'];
                $where['com_id']=$this->data['com_id'];
                echo $this->mdl_socialmedia->update_data($where,$data);
            }
            else //add
            {
                echo $this->mdl_socialmedia->add_data($data);
            }
        }
        else
    ...
    function view_data()
    {
        $where['com_id']=$this->data['com_id'];
        if (isset($_GET['id']))
	         $where['socialmedia_id']=$_GET['id'];
        
        if (isset($_GET['data']))
	        $select=$_GET['data'];
	    else $select="*";
	    
        $return=$this->mdl_socialmedia->view_data($where,$select);
    ...
    function delete_data()
    {
        if (isset($_GET['id']) && $_GET['id'])
        {
            $where['com_id']=$this->data['com_id'];
            $where['socialmedia_id']=$_GET['id'];
            $object=json_encode($this->mdl_socialmedia->view_data($where,"*")->result());
            $data_title= "socialmedia  Delete";
            
            $this->load->module("logs");
            if ($this->logs->add_data($data_title,$object)) {
                echo $this->mdl_socialmedia->delete_data($where);
            }
        }
    }
    ...
    {
        $path1="./assets/website/uploads/socialmedia/".$name;
        unlink($path1);
        $path2="./assets/website/uploads/socialmedia/thumb/".$name;
        unlink($path2);
    }
}
?>