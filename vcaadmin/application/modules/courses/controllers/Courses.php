...
class Courses extends MX_Controller
{
    ...
    function save_data()
    {
        ...
        if ($this->form_validation->run()==TRUE)
        {
            ...
            if (isset($_POST['c_id']) && $_POST['c_id'])//update
            {
                ...
                echo $this->mdl_courses->update_data($where,$data);
            }
            else //add
            {
                echo $this->mdl_courses->add_data($data);
            }
        }
        else
        {
    ...
    function view_data()
    {
        ...
        $return=$this->mdl_courses->view_data($where,$select);
    ...
    function delete_data()
        {
            ...
            {
                $where['c_id']=$_GET['id'];
                $object=json_encode($this->mdl_courses->view_data($where,"*")->result());
                $data_title= "Video  Delete";
                
                $this->load->module("logs");
                if ($this->logs->add_data($data_title,$object)) {
                    echo $this->mdl_courses->delete_data($where);
                }
            }
        }
        ...
        function remove_image($title)
        {
            if (substr($title, 0,4)!="http"){
                $path1="./assets/website/uploads/courses/".$title;
                unlink($path1);
                $path2="./assets/website/uploads/courses/thumb/".$title;
                unlink($path2);
            }
        }
