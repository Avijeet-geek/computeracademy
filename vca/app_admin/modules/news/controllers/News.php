...
class News extends MX_Controller
{
    ...
    function save_data()
    {
        ...
        $this->form_validation->set_rules("deleted_price","Deleted Price","required|trim");
        ...
        if ($this->form_validation->run()==TRUE)
        {
            ...
            $data['deleted_price']=$_POST['deleted_price'];
...
// print_r($data);die();
    ...
    function image_upload($title)
    {
        ...
        $config['new_image'] = './assets/website/uploads/news/';
        ...
        else
        {
            ...
            $config['source_image'] = './assets/website/temp/' . $image['file_name'];
            $config['new_image'] = './assets/website/uploads/news/thumb/';
            $config['file_name'] = $title . "_" . $rand_number . $timestamp;
            $config['maintain_ratio'] = TRUE;
            if ($image['image_width']>500){
                $config['width'] = 500;
            }
            $config['overwrite'] = FALSE;
            $this->load->library('image_lib', $config);
    ...
    {
        $path1="./assets/website/uploads/news/".$name;
        unlink($path1);
        $path2="./assets/website/uploads/news/thumb/".$name;
        unlink($path2);
    }
}
?>